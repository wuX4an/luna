...
func Tan(L *lua.LState) int {
	...
	switch lstring {
	case "r":
		result = math.Tan(float64(lnumber))
	case "g":
		if lnumber == 90 {
			L.RaiseError(
				"mathematical error: tangent is undefined at 90 degrees",
			)
		}
		result = math.Tan(float64(lnumber) * math.Pi / 180)
	}
